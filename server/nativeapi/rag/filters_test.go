...
func TestBuildQdrantFilterExactFilters(t *testing.T) {
	...
	want := map[string]any{"must": []any{
		...
		map[string]any{"key": "genre", "match": map[string]any{"value": "Pop"}},
...
func TestBuildQdrantFilterRangeFilters(t *testing.T) {
	...
	filter := BuildQdrantFilter(SearchFilters{
		...
		PlayCountMax: &playCountMax, DurationMax: &durationMax,
	})
	want := map[string]any{"must": []any{
		...
		map[string]any{"key": "playCount", "range": map[string]any{"lte": int64(20)}},
		map[string]any{"key": "duration", "range": map[string]any{"lte": 240.0}},
	}}
	if !reflect.DeepEqual(filter, want) {
