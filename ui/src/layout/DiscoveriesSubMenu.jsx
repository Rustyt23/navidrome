...
const fetchDiscoveryTrackIds = async (playlistId) => {
...
const filterSongDropPayload = async (playlistId, item) => {
  ...
  const existingIds = await fetchDiscoveryTrackIds(playlistId)
  const seen = new Set()
  const uniqueIds = []
  ...
  const handleDrop = useCallback(
    async (item) => {
      ...
      return dataProvider
        ...
        .then((res) => notify('message.songsAddedToDiscovery', 'info', { smart_count: res?.data?.added }))
...
const FolderRow = memo(function FolderRow({
    ...
    try {
      await ensure(node.id, async () => {
        const res = await dataProvider.getList('folder', {
          pagination: { page: 1, perPage: config.maxSidebarDiscoveryFolders },
  ...
  const childrenTyped = useMemo(() => ({
    ...
    discoveries: (items || []).filter((i) => i.type === 'playlist'),
  }), [items])
    ...
    <>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        <List disablePadding className={classes.nested}>
          ...
          {childrenTyped.discoveries.map((p) => (
...
const DiscoveriesSubMenu = ({ state, setState, sidebarIsOpen, dense }) => {
  const history = useHistory()
  const dataProvider = useDataProvider()
  const notify = useNotify()
  const refresh = useRefresh()
  ...
  const discoveries = useMemo(() => (rootItems || []).filter((i) => i.type === 'playlist'), [rootItems])
...
export default DiscoveriesSubMenu
