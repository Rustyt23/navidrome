...
type Discoveries interface {
	ImportFile(ctx context.Context, folder *model.Folder, filename string) (*model.Discovery, error)
	Update(ctx context.Context, playlistID string, name *string, comment *string, public *bool, idsToAdd []string, idxToRemove []int) error
	SetFolder(ctx context.Context, playlistID string, folderID *string) error
	ImportM3U(ctx context.Context, reader io.Reader) (*model.Discovery, error)
...
func (s *discoveries) ensureDiscoveryFolder(ctx context.Context, playlistPath string) (*string, error) {
	if conf.Server.PlaylistsPath == "" {
		return nil, nil
	}
	...
	for _, root := range paths {
		...
		for _, p := range parts {
			...
			if len(folders) > 0 {
				...
				f := &model.DiscoveryFolder{Name: p, OwnerID: owner.ID, Public: conf.Server.DefaultPlaylistPublicVisibility}
				if parentID != nil {
					f.ParentID = parentID
				}
...
func (s *discoveries) ImportFile(ctx context.Context, folder *model.Folder, filename string) (*model.Discovery, error) {
	pls, err := s.parsePlaylist(ctx, filename, folder)
	if err != nil {
		log.Error(ctx, "Error parsing discovery", "path", filepath.Join(folder.AbsolutePath(), filename), err)
		return nil, err
	}
	if fid, err := s.ensureDiscoveryFolder(ctx, pls.Path); err == nil {
		pls.FolderID = fid
	} else {
		log.Error(ctx, "Error ensuring discovery folder", "path", pls.Path, err)
	}
	log.Debug("Found discovery", "name", pls.Name, "lastUpdated", pls.UpdatedAt, "path", pls.Path, "numTracks", len(pls.Tracks))
	err = s.updatePlaylist(ctx, pls)
	if err != nil {
		log.Error(ctx, "Error updating discovery", "path", filepath.Join(folder.AbsolutePath(), filename), err)
	}
	return pls, err
}

func (s *discoveries) ImportM3U(ctx context.Context, reader io.Reader) (*model.Discovery, error) {
	owner, _ := request.UserFrom(ctx)
	pls := &model.Discovery{
		OwnerID: owner.ID,
		Public:  false,
	...
	if err != nil {
		log.Error(ctx, "Error parsing discovery", err)
		return nil, err
	}
	err = s.ds.Discovery(ctx).Put(pls)
	if err != nil {
		log.Error(ctx, "Error saving discovery", err)
		return nil, err
	}
	return pls, nil
}

func (s *discoveries) parsePlaylist(ctx context.Context, playlistFile string, folder *model.Folder) (*model.Discovery, error) {
...
func (s *discoveries) newSyncedPlaylist(baseDir string, playlistFile string) (*model.Discovery, error) {
...
func (s *discoveries) parseNSP(_ context.Context, pls *model.Discovery, reader io.Reader) error {
	nsp := &nspFile{}
...
func (s *discoveries) parseM3U(ctx context.Context, pls *model.Discovery, folder *model.Folder, reader io.Reader) error {
	...
	for lines := range slice.CollectChunks(slice.LinesFrom(reader), 400) {
		...
		if err != nil {
			log.Warn(ctx, "Error normalizing paths in discovery", "discovery", pls.Name, err)
			continue
		}
		found, err := mediaFileRepository.FindByPaths(paths)
		if err != nil {
			log.Warn(ctx, "Error reading files from DB", "discovery", pls.Name, err)
			continue
		}
		...
		for _, path := range paths {
			...
			if ok {
				...
				log.Warn(ctx, "Path in discovery not found", "discovery", pls.Name, "path", path)
			}
		}
	}
	if pls.Name == "" {
...
func (s *discoveries) normalizePaths(ctx context.Context, pls *model.Discovery, folder *model.Folder, lines []string) ([]string, error) {
	...
	for idx, line := range lines {
		...
		if found {
			...
			log.Warn(ctx, "Path in discovery not found in any library", "path", line, "line", idx)
		}
	}
	return slice.Map(res, filepath.ToSlash), nil
}

func (s *discoveries) updatePlaylist(ctx context.Context, newPls *model.Discovery) error {
	owner, _ := request.UserFrom(ctx)

	pls, err := s.ds.Discovery(ctx).FindByPath(newPls.Path)
	if err != nil && !errors.Is(err, model.ErrNotFound) {
		return err
	}
	if err == nil && !pls.Sync {
		log.Debug(ctx, "Playlist already imported and not synced", "discovery", pls.Name, "path", pls.Path)
		return nil
	}

	if err == nil {
		log.Info(ctx, "Updating synced discovery", "discovery", pls.Name, "path", newPls.Path)
		newPls.ID = pls.ID
		...
		log.Info(ctx, "Adding synced discovery", "discovery", newPls.Name, "path", newPls.Path, "owner", owner.UserName)
		newPls.OwnerID = owner.ID
		newPls.Public = conf.Server.DefaultPlaylistPublicVisibility
	}
	return s.ds.Discovery(ctx).Put(newPls)
}

func (s *discoveries) Update(ctx context.Context, playlistID string,
...
func (s *discoveries) SetFolder(ctx context.Context, playlistID string, folderID *string) error {
...
func (s *discoveries) writePlaylistFile(path string, pls *model.Discovery, mirrorToSync bool) error {
...
func (s *discoveries) Publish(ctx context.Context, playlistID string) error {
	return s.ds.WithTxImmediate(func(tx model.DataStore) error {
		repo := tx.Discovery(ctx)
		pls, err := repo.GetWithTracks(playlistID, true, false)
		if err != nil {
		...
		if !pls.Sync {
			return fmt.Errorf("discovery is not synced")
		}
		if pls.Path == "" {
...
func (s *discoveries) buildPlaylistPath(ctx context.Context, ds model.DataStore, folderID *string, name, ext string) (string, error) {
	if conf.Server.PlaylistsPath == "" {
		return "", fmt.Errorf("discoveries path not configured")
	}
	paths := strings.Split(conf.Server.PlaylistsPath, string(filepath.ListSeparator))
