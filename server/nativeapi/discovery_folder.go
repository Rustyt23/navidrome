...
func ListFoldersAndPlaylists(ds model.DataStore) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		folders, err := ds.DiscoveryFolder(ctx).GetAllByParent(opts.FolderOpts)
		if err != nil {
			http.Error(w, err.Error(), statusFor(err))
			return
		}

		playlists, err := ds.Discovery(ctx).GetAllByPlaylistFolder(opts.PlaylistOpts)
		if err != nil {
			http.Error(w, err.Error(), statusFor(err))
			return
		}

		commons := make(model.DiscoveryAndFolderCommons, 0, len(folders)+len(playlists))
...
func BulkMove(ds model.DataStore, pls core.Discoveries) http.HandlerFunc {
