...
func CreateNativeAPIRouter(ctx context.Context) *nativeapi.Router {
	...
	modelScanner := scanner.New(ctx, dataStore, cacheWarmer, broker, playlistsPlaylists, discoveries, metricsMetrics)
	watcher := scanner.GetWatcher(dataStore, modelScanner)
	library := core.NewLibrary(dataStore, modelScanner, watcher, broker, manager)
	user := core.NewUser(dataStore, manager)
	maintenance := core.NewMaintenance(dataStore)
	router := nativeapi.New(dataStore, mediaStreamer, share, playlistsPlaylists, insights, library, user, maintenance, manager, imageUploadService, modelScanner)
	return router
}
