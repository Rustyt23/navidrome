...
// the response straight to disk, so a selection of a few hundred megabytes costs
// nothing in memory. Fetching it instead would mean holding the whole archive in
// the tab as a blob before the save dialog ever appeared.
//
// The token rides in the query string because a navigation cannot carry headers.
// The auth chain already accepts it there - it is how the event stream
...
export const DownloadSongsButton = ({ selectedIds, className, disabled }) => {
  ...
  const handleClick = useCallback(() => {
    ...
    // raw: these pages exist to show what normalization did to the actual files,
    // and handing back a transcode would answer a question nobody asked.
