...
// request that times out somewhere in the middle and leaves a truncated
...
func (n *Router) downloadSongs() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := archiver.ZipMediaFiles(ctx, ids, format, bitrate, w); err != nil {
			// Deliberately not written to the response: the headers went out
			// with the first byte of the zip, so anything added here would be
			// appended to the archive rather than shown to anyone. The log is
			// the only honest place left to say it.
