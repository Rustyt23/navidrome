...
func (api *Router) Stream(w http.ResponseWriter, r *http.Request) (*responses.Subsonic, error) {
	...
	mf, err := api.getMediaFileForStream(ctx, id)
	if err != nil {
		return nil, err
	}
...
// getMediaFileForStream resolves an ID to a MediaFile, supporting the fork's
// discovery-track stream IDs in addition to regular media file IDs.
func (api *Router) getMediaFileForStream(ctx context.Context, id string) (*model.MediaFile, error) {
	if discID, trackID, ok := model.ParseDiscoveryStreamID(id); ok {
		entry, err := api.ds.Discovery(ctx).Tracks(discID).Read(trackID)
		if err != nil {
			return nil, err
		}
	...
	return api.ds.MediaFile(ctx).Get(id)
}
