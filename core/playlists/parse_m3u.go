...
func (s *playlists) parseM3U(ctx context.Context, pls *model.Playlist, folder *model.Folder, reader io.Reader) error {
	...
	for lines := range slice.CollectChunks(slice.LinesFrom(reader), 100) {
		...
		resolvedPaths, err := resolver.resolvePaths(ctx, folder, pls.Path, filteredLines)
		if err != nil {
...
// findByGlob searches all libraries for a file matching the entry's base name.
// This tolerates playlist entries whose directory layout doesn't match any library.
func (r *pathResolver) findByGlob(line string) pathResolution {
	base := filepath.Base(filepath.Clean(line))
	if base == "" || base == "." || base == string(filepath.Separator) {
		return pathResolution{valid: false}
	}
	for _, lib := range r.matcher.libraries {
		cleanLibPath := filepath.Clean(lib.Path)
		pattern := filepath.Join(cleanLibPath, "**", base)
		matches, _ := doublestar.FilepathGlob(pattern)
		for _, m := range matches {
			rel, err := filepath.Rel(cleanLibPath, m)
			if err != nil || inPlaylistsPath(rel) {
				continue
			}
			return pathResolution{absolutePath: m, libraryPath: cleanLibPath, libraryID: lib.ID, valid: true}
		}
	}
	return pathResolution{valid: false}
}
...
func (r *pathResolver) resolvePaths(ctx context.Context, folder *model.Folder, playlistPath string, lines []string) ([]string, error) {
	results := make([]string, 0, len(lines))
	for idx, line := range lines {
		resolution := r.resolvePath(line, folder)

		if resolution.valid {
			// Skip entries that point inside the configured PlaylistsPath (e.g. other playlist files)
			if rel, err := filepath.Rel(resolution.libraryPath, resolution.absolutePath); err == nil && inPlaylistsPath(rel) {
				continue
			}
		} else {
			// Fallback: search all libraries for a file with the same base name. This
			// tolerates playlists created on other machines with different path layouts.
			resolution = r.findByGlob(line)
		}

		if !resolution.valid {
			log.Warn(ctx, "Path in playlist not found in any library", "path", line, "line", idx)
			recordMissingPlaylistTrack(ctx, playlistPath, line)
			continue
		}
