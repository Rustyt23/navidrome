...
func (s *playlists) Update(ctx context.Context, playlistID string,
	...
	return s.ds.WithTxImmediate(func(tx model.DataStore) error {
		if err := s.updateTx(ctx, tx, playlistID, pls, name, comment, public, idsToAdd, idxToRemove); err != nil {
			return err
		}
		if pls.Sync {
			return s.syncPlaylistToDisk(ctx, tx, playlistID)
		}
		return nil
	})
}

func (s *playlists) updateTx(ctx context.Context, tx model.DataStore, playlistID string, pls *model.Playlist,
	name *string, comment *string, public *bool,
	idsToAdd []string, idxToRemove []int) error {
	{
