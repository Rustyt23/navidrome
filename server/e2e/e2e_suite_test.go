...
func (n noopArchiver) ZipMediaFiles(context.Context, []string, string, int, io.Writer) error {
	return model.ErrNotFound
}
...
var _ = BeforeSuite(func() {
	...
	s := scanner.New(ctx, initDS, artwork.NoopCacheWarmer(), events.NoopBroker(),
		playlists.NewPlaylists(initDS, core.NewImageUploadService()), core.NewDiscoveries(initDS), metrics.NewNoopInstance())
